...
public class DBLogin {
    public static boolean signIn(String name, String password) {
        MyThread myThread = new MyThread("http://47.95.2.241:8080/app/SignIn",name,password);
        try
        {
            myThread.start();
            myThread.join();
    ...
    public static boolean signUp(String name, String password) {
        MyThread myThread = new MyThread("http://47.95.2.241:8080/app/SignUp",name,password);
        try
        {
            myThread.start();
            myThread.join();
